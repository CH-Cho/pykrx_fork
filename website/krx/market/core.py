...
class 투자자별_거래실적_개별종목_기간합계(KrxWebIo):
    ...
    def fetch(self, strtDd: str, endDd: str, isuCd: str) -> DataFrame:
        ...
        Browser > Network > Payload > Form Data
        bld=dbms/MDC/STAT/standard/MDCSTAT02301&
        locale=ko_KR&inqTpCd=1&
        trdVolVal=2&
        askBid=3&
        tboxisuCd_finder_stkisu0_2=005930%2F%EC%82%BC%EC%84%B1%EC%A0%84%EC%9E%90&
        isuCd=KR7005930003&
        isuCd2=KR7005930003&
        codeNmisuCd_finder_stkisu0_2=%EC%82%BC%EC%84%B1%EC%A0%84%EC%9E%90&
...
class 개별지수시세_파라미터(KrxWebIo):
    @property
    def bld(self):
        return "dbms/MDC/STAT/standard/MDCSTAT00301"

    def fetch(self, indIdx: str, indIdx2: str, strtDd: str, endDd: str,
              share: str = None, money: str = None) -> DataFrame:
        """[11003] 개별지수 시세 추이 (확장 파라미터)

        Args:
            indIdx   (str): index group id (예: '1' KOSPI, '2' KOSDAQ)
            indIdx2  (str): index ticker (예: '001')
            strtDd   (str): 조회 시작 일자 (YYYYMMDD)
            endDd    (str): 조회 종료 일자 (YYYYMMDD)
            share    (str): (선택) 지수 산출 보정 옵션
            money    (str): (선택) 금액 단위 옵션

        Returns:
            DataFrame: 원본 출력 블록을 반환
        """
        params = dict(indIdx=indIdx, indIdx2=indIdx2, strtDd=strtDd, endDd=endDd)
        if share is not None:
            params['share'] = share
        if money is not None:
            params['money'] = money
        result = self.read(**params)
        return DataFrame(result.get('output') or result.get('OutBlock_1') or result.get('block1') or [])
