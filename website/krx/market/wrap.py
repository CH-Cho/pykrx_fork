...
# raw-style wrappers (원본 열 구조 유지 + 최소 rename)
from pykrx.website.comm import dataframe_empty_handler


@dataframe_empty_handler
def get_kospi_index_by_date(fromdate: str, todate: str,
                                share: str = "2", money: str = "3") -> DataFrame:
    """코스피지수 개별 시세 (원본 구조 유지)

    Args:
        fromdate (str): YYYYMMDD
        todate   (str): YYYYMMDD
        share    (str): (선택) 분할 옵션. 기본 '2'
        money    (str): (선택) 금액 단위. 기본 '3'
    """
    df = 개별지수시세_파라미터().fetch(
        indIdx="1", indIdx2="001", strtDd=fromdate, endDd=todate,
        share=share, money=money
    )
    return df.rename(columns={"TRD_DD": "Date"})


@dataframe_empty_handler
def get_kosdaq_index_by_date(fromdate: str, todate: str,
                                 share: str = "2", money: str = "3") -> DataFrame:
    """코스닥지수 개별 시세 (원본 구조 유지)
    ...
    """
    df = 개별지수시세_파라미터().fetch(
        indIdx="2", indIdx2="001", strtDd=fromdate, endDd=todate,
        share=share, money=money
    )
    return df.rename(columns={"TRD_DD": "Date"})
