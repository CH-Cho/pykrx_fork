...
def get_kospi_index_by_date(fromdate: str, todate: str,
    ...
    Args:
        ...
        todate   (str): YYYYMMDD
        share    (str): 분할 옵션. 기본 '2'
        money    (str): 금액 단위. 기본 '3'
    """
    if isinstance(fromdate, datetime.datetime):
        fromdate = krx.datetime2string(fromdate)
    if isinstance(todate, datetime.datetime):
    ...
    df = krx.get_kospi_index_by_date(fromdate, todate, share, money)
    return df
...
def get_kosdaq_index_by_date(fromdate: str, todate: str,
    ...
    fromdate = str(fromdate).replace("-", "").replace("/", "")
    todate = str(todate).replace("-", "").replace("/", "")
    df = krx.get_kosdaq_index_by_date(fromdate, todate, share, money)
    return df
